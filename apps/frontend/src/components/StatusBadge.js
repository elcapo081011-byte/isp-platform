...
export function StatusBadge({ status }) {
    return (_jsx("span", { className: `inline-block text-xs px-2 py-0.5 rounded border whitespace-nowrap ${STYLES[status] ?? STYLES.UNKNOWN}`, children: LABELS[status] ?? status }));
}
